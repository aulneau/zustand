...
const redux = (reducer, initial) => (set, get, api) => {
  ...
  return __assign({ dispatch: api.dispatch }, initial)
}
  ...
  if (!api.devtools) {
    ...
    api.devtools.subscribe((message) => {
      ...
      if (message.type === 'DISPATCH' && message.state) {
        ...
        ((_a = message.payload) == null ? void 0 : _a.type) === 'COMMIT'
      ) {
...
const combine = (initialState, create) => (set, get, api) =>
  Object.assign({}, initialState, create(set, get, api))
const persist = (config, options) => (set, get, api) => {
  ...
  ;(async () => {
    const postRehydrationCallback =
      (onRehydrateStorage == null ? void 0 : onRehydrateStorage(get())) ||
      void 0
    try {
      ...
      if (storageValue) {
        ...
        if (deserializedStorageValue.version !== version) {
          const migratedState = await (migrate == null
            ? void 0
            : migrate(
